...
def load_data_shared(filename="../data/mnist.pkl.gz"):
    ...
    training_data = data_loader.load_train1(filename)
    print 'load training data finished'
    f = gzip.open("../data/mnist.pkl.gz", 'rb')
    validation_data, _, _ = cPickle.load(f)
    f.close()
    print 'load validation data finished'
    test_data = data_loader.load_sample()
    print 'load sample data finished'

    # print len(validation_data[0])

    # import data_loader
    # training_data = data_loader.load_train(filename)
    ...
    #
    # training_data = [tmp_data[0][:9000], tmp_data[1][:9000]]
    # validation_data = [tmp_data[0][:4000], tmp_data[1][:4000]]
        ...
        for epoch in xrange(epochs):
            for minibatch_index in xrange(num_training_batches):
                ...
                if iteration % 3000 == 0:
                    ...
                    if best_validation_accuracy > 0.9925:
                        break;
            print (best_validation_accuracy)
            if best_validation_accuracy > 0.9925:
                break;
