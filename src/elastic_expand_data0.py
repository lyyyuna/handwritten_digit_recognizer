...
f = gzip.open("../data1/shift_rotate_expand1.pkl.gz", 'rb')
...
for x, y in zip(training_data[0], training_data[1]):
    digit = int(y)
    j += 1
    if j % 100 == 0: print("Expanding image number", j)

    expanded_training_pairs.append((numpy.reshape(x, 784), digit))

    image = np.reshape(x, (-1, 28))

    ran_num = random.uniform(5.5, 8)

    distorted_image = elastic_transform(image, kernel_dim=15, alpha=ran_num, sigma=35, negated=True)
    mask = np.where(distorted_image < 60)
    for i in range(0, len(mask[0])):
        distorted_image[mask[0][i], mask[1][i]] = 0

    expanded_training_pairs.append((np.reshape(distorted_image, 784), digit))

random.shuffle(expanded_training_pairs)
expanded_training_data = [list(d) for d in zip(*expanded_training_pairs)]
print("Saving expanded data. This may take a few minutes.")
f = gzip.open("../data1/elastic_rotate_expand.pkl.gz", "wb")
cPickle.dump(expanded_training_data, f)
f.close()


# f = open('../train/0a1fa42f', 'rb')
# ss =''
# for i in range(0, 784):
#     num = f.read(1)
#     if i % 28 == 0:
#         ss += '\n'
#     if int(num.encode('hex'), 16) > 0:
#         ss += ' '
#     else:
#         ss += '#'
# print ss
#
# dt = numpy.dtype('B')
# x = numpy.fromfile('../train/0a1fa42f', dt)
# image = np.reshape(x, (-1, 28))
#
# print
# distorted_image = elastic_transform(image, kernel_dim=15, alpha=5.5, sigma=35, negated=True)
#
#
# nimg = numpy.reshape(distorted_image, (1, 784))
# nimg = nimg.tolist()
#
# print
# ss = ''
...
# else:
#     f = gzip.open("../data/expanded01.pkl.gz", 'rb')
#     training_data = cPickle.load(f)
...
#
#         distorted_image = elastic_transform(image, kernel_dim=15, alpha=5.5, sigma=35)
#
#
#         expanded_training_pairs.append((np.reshape(distorted_image, 784), y))
#
#     random.shuffle(expanded_training_pairs)
#     expanded_training_data = [list(d) for d in zip(*expanded_training_pairs)]
#     print("Saving expanded data. This may take a few minutes.")
#     f = gzip.open("../data/distort_expanded01.pkl.gz", "w")
#     cPickle.dump(expanded_training_data, f)
#     f.close()
